...
def render_upload_tab(pdf_processor, processor, db_connection):
    ...
    # Use tabs for different upload types
    pdf_tab, csv_tab = st.tabs(["Upload PDF", "Upload CSV"])
    ...
    with pdf_tab:
        ...
        if uploaded_pdf_file is not None:
            ...
            with col1:
                st.info(f"📄 File: {uploaded_pdf_file.name}")
                st.info(f"📏 Size: {uploaded_pdf_file.size:,} bytes")
            ...
            with col2:
                ...
                if st.button("🚀 Process PDF", type="primary", key="process_pdf_button"):
                    with st.spinner("Processing PDF... This may take a moment..."):
                        try:
                            # Process the PDF
                            json_data = pdf_processor.process_pdf(uploaded_pdf_file)
                            if json_data:
                ...
                if st.session_state.processed_json:
                    with st.status("Database Upload Status", expanded=True) as status:
                        if st.button("💾 Save to Database", key="save_pdf_to_db_button"):
                            try:
                                ...
                                st.session_state.processed_json['processed_by'] = 'streamlit_app_pdf'
    ...
    with csv_tab:
        ...
        uploaded_csv_file = st.file_uploader(
            ...
            type=['csv'],
            help="Upload a CSV bank statement to directly upload to database",
            key="csv_uploader"
        )
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        if uploaded_csv_file is not None:
            ...
            with col1:
                st.info(f"📄 File: {uploaded_csv_file.name}")
                st.info(f"📏 Size: {uploaded_csv_file.size:,} bytes")

            with col2:
                if st.button("⬆️ Upload CSV to Database", type="primary", key="upload_csv_button"):
                    with st.spinner("Uploading CSV... This may take a moment..."):
                        try:
                            # Read CSV
                            ...
                            with st.status("Database Upload Status", expanded=True) as status:
                                status.write("🛢️ Starting MongoDB upload process for CSV...")
                                status.write(f"📊 Number of rows to upload: {len(data_to_upload)}")
                                status.write(f"📄 Filename: {uploaded_csv_file.name}")
                                ...
                                for record in data_to_upload:
                                    ...
                                    record['processed_by'] = 'streamlit_app_csv'
                                    record['original_filename'] = uploaded_csv_file.name
                                    inserted_id = db_connection.insert_document(record)
                                    if inserted_id:
                                        inserted_count += 1
                                    else:
                                        st.warning(f"Could not insert record: {record}")

                                status.write(f"📝 Inserted {inserted_count} documents.")

                                # Verify insertion
                                doc_count = db_connection.count_documents()
                                status.write(f"📊 Total documents in collection: {doc_count}")

                                status.update(label="✅ CSV data uploaded to MongoDB successfully!", state="complete")
